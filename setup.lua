...
Rocks installation path: [install_path:50:{{vim.fs.normalize(vim.fn.stdpath('data'), "rocks")}}]
...
local function set_up_luarocks(install_path)
	...
	local tempdir = "/tmp/rock-temp-dir"
	...
	vim.keymap.set("n", "<CR>", function()
		...
		if line == "< OK >" then
			...
			if setup_luarocks then
				...
				luarocks_binary = vim.fs.normalize(install_path, "bin", "luarocks")
			...
			acquire_buffer_lock(buffer, function()
				vim.api.nvim_buf_set_lines(buffer, 0, -1, true, {
					...
					" ",
					" local luarocks_path = {",
					'     vim.fs.normalize(rocks_config.rocks_path, "share", "lua", "5.1", "?.lua"),',
					'     vim.fs.normalize(rocks_config.rocks_path, "share", "lua", "5.1", "?", "init.lua"),',
					" }",
					' package.path = package.path .. ";" .. table.concat(luarocks_path, ";")',
					" ",
					" local luarocks_cpath = {",
					'     vim.fs.normalize(rocks_config.rocks_path, "lib", "lua", "5.1", "?.so"),',
					'     vim.fs.normalize(rocks_config.rocks_path, "lib64", "lua", "5.1", "?.so"),',
					" }",
					' package.cpath = package.cpath .. ";" .. table.concat(luarocks_cpath, ";")',
					" ",
					' vim.opt.runtimepath:append(vim.fs.normalize(rocks_config.rocks_path, "lib", "luarocks", "rocks-5.1", "rocks.nvim", "*"))',
					"<",
				...
				vim.fn.setreg('"', {
					...
					'package.cpath = package.cpath .. ";" .. table.concat(luarocks_cpath, ";")',
					"",
					'vim.opt.runtimepath:append(vim.fs.normalize(rocks_config.rocks_path, "lib", "luarocks", "rocks-5.1", "*", "*"))',
					---@diagnostic disable-next-line: param-type-mismatch
				}, "l")
		...
		elseif line == "<< OPEN INIT.LUA >>" then
			---@diagnostic disable-next-line: param-type-mismatch
			vim.cmd.edit(vim.fs.normalize(vim.fn.stdpath("config"), "init.lua"))
			vim.cmd("write ++p")
